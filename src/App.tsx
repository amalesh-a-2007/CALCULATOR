...
import { 
  History, RotateCcw, Plus, Trash2, HelpCircle, Sparkles, 
  ChevronRight, Divide, Percent, DollarSign, ArrowUpRight, ArrowDownRight, LayoutGrid 
} from "lucide-react";
  ...
  const handleAccountingModalSubmit = (e: React.FormEvent) => {
    ...
    try {
      ...
      switch (accountingAction) {
        ...
        case "loss":
          // Loss Margin/Spread
          resultVal = String(+(((inputVal - currentVal) / inputVal) * 100).toFixed(4));
          break;
        case "markup":
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_210px] gap-2.5 items-start">
          ...
          <div className="space-y-2.5">
            ...
            <div className="bg-ink2 border border-border-custom rounded-t-xl p-4 pb-3 relative overflow-hidden flex flex-col select-none border-b-0">
              ...
              <div className="text-[10px] text-dim-custom min-h-[16px] mb-0.5 text-right overflow-hidden text-ellipsis whitespace-nowrap" id="display-steps-log">
                {prevExprString}
              </div>

              {/* LIVE DYNAMIC EXPR SYNCHRONIZATION */}
              <div className="text-[12px] text-accent-custom/70 min-h-[18px] text-right overflow-hidden text-ellipsis whitespace-nowrap" id="display-expr-sync">
              ...
              <div className="flex justify-between items-center mt-1.5">
                ...
                <span 
                  className={`text-[9px] font-mono tracking-wide transition-all ${
                    memory !== null ? "text-yellow-custom drop-shadow-[0_0_10px_rgba(255,209,102,0.35)] font-bold" : "text-dim-custom"
                  }`}
            ...
            <div className="bg-ink2 border border-border-custom rounded-b-xl p-2.5 space-y-2.5">
              ...
              {activePanel === "main" && (
                <div className="grid grid-cols-5 gap-1.5 p-0.5 animate-bounce-in" id="panel-kb-main">
                  {/* MEMORY STACK ROW ACTIONS */}
                  <button className="calc-btn btn-mem py-3 bg-ink3 border border-border-custom text-[11px] font-mono hover:bg-ink4 transition-all rounded-md cursor-pointer text-yellow-custom hover:border-yellow-custom/40" onClick={handleMemoryClear}>MC</button>
                  <button className="calc-btn btn-mem py-3 bg-ink3 border border-border-custom text-[11px] font-mono hover:bg-ink4 transition-all rounded-md cursor-pointer text-yellow-custom hover:border-yellow-custom/40" onClick={handleMemoryRecall}>MR</button>
                  <button className="calc-btn btn-mem py-3 bg-ink3 border border-border-custom text-[11px] font-mono hover:bg-ink4 transition-all rounded-md cursor-pointer text-yellow-custom hover:border-yellow-custom/40" onClick={handleMemoryStore}>MS</button>
                  <button className="calc-btn btn-mem py-3 bg-ink3 border border-border-custom text-[11px] font-mono hover:bg-ink4 transition-all rounded-md cursor-pointer text-yellow-custom hover:border-yellow-custom/40" onClick={handleMemoryAdd}>M+</button>
                  <button className="calc-btn btn-mem py-3 bg-ink3 border border-border-custom text-[11px] font-mono hover:bg-ink4 transition-all rounded-md cursor-pointer text-yellow-custom hover:border-yellow-custom/40" onClick={handleMemorySubtract}>M-</button>

                  {/* CORE FUNCTIONS */}
                  <button className="calc-btn btn-fn py-3 bg-ink3 border border-border-custom text-[11px] font-semibold text-green-custom hover:border-green-custom/40 transition-all rounded-md cursor-pointer" onClick={() => handleInsertCharacter("Math.abs(")}>|x|</button>
                  <button className="calc-btn btn-fn py-3 bg-ink3 border border-border-custom text-[11px] font-semibold text-green-custom hover:border-green-custom/40 transition-all rounded-md cursor-pointer" onClick={() => handleInsertCharacter("Math.round(")}>RND</button>
                  <button className="calc-btn btn-fn py-3 bg-ink3 border border-border-custom text-[11px] font-semibold text-green-custom hover:border-green-custom/40 transition-all rounded-md cursor-pointer" onClick={() => handleInsertCharacter("Math.floor(")}>FLR</button>
                  <button className="calc-btn btn-fn py-3 bg-ink3 border border-border-custom text-[11px] font-semibold text-green-custom hover:border-green-custom/40 transition-all rounded-md cursor-pointer" onClick={() => handleInsertCharacter("Math.ceil(")}>CEIL</button>
                  <button className="calc-btn btn-fn py-3 bg-ink3 border border-border-custom text-[11px] font-semibold text-green-custom hover:border-green-custom/40 transition-all rounded-md cursor-pointer" onClick={() => handleInsertCharacter("%")}>MOD</button>

                  <button className="calc-btn bg-red-custom/5 border border-red-custom/15 text-red-custom/90 font-mono text-[11px] py-3 rounded-md hover:bg-red-custom/10 hover:border-red-custom cursor-pointer transition-all" onClick={handleAllClear}>AC</button>
                  <button className="calc-btn bg-red-custom/5 border border-red-custom/15 text-red-custom/90 font-mono text-[11px] py-3 rounded-md hover:bg-red-custom/10 hover:border-red-custom cursor-pointer transition-all" onClick={handleBackspace}>CE</button>
                  <button className="calc-btn cursor-pointer py-3 bg-ink3 border border-border-custom text-white hover:bg-ink4 rounded-md transition-all font-bold text-center" onClick={handleBackspace}>⌫</button>
                  <button className="calc-btn flex items-center justify-center cursor-pointer py-3 bg-accent-custom/5 border border-accent-custom/15 text-accent-custom hover:border-accent-custom hover:bg-ink4 rounded-md transition-all font-mono" onClick={() => handleInsertCharacter("/")}>÷</button>
                  <button className="calc-btn flex items-center justify-center cursor-pointer py-3 bg-accent-custom/5 border border-accent-custom/15 text-accent-custom hover:border-accent-custom hover:bg-ink4 rounded-md transition-all font-mono" onClick={() => handleInsertCharacter("**")}>xʸ</button>

                  {/* DIGIT TILE ROWS & BASIC OPERATORS */}
                  <button className="calc-btn text-base py-3 bg-ink3 border border-border-custom font-medium hover:bg-ink4 transition-all rounded-md cursor-pointer text-white" onClick={() => handleInsertCharacter("7")}>7</button>
                  <button className="calc-btn text-base py-3 bg-ink3 border border-border-custom font-medium hover:bg-ink4 transition-all rounded-md cursor-pointer text-white" onClick={() => handleInsertCharacter("8")}>8</button>
                  <button className="calc-btn text-base py-3 bg-ink3 border border-border-custom font-medium hover:bg-ink4 transition-all rounded-md cursor-pointer text-white" onClick={() => handleInsertCharacter("9")}>9</button>
                  <button className="calc-btn py-3 bg-accent-custom/5 border border-accent-custom/15 text-accent-custom hover:border-accent-custom hover:bg-ink4 rounded-md transition-all font-bold text-base flex items-center justify-center" onClick={() => handleInsertCharacter("*")}>&times;</button>
                  <button className="calc-btn text-sm py-3 bg-ink3 border border-border-custom text-green-custom hover:bg-ink4 hover:border-green-custom/30 transition-all rounded-md cursor-pointer font-semibold" onClick={() => handleInsertCharacter("Math.sqrt(")}>√</button>

                  <button className="calc-btn text-base py-3 bg-ink3 border border-border-custom font-medium hover:bg-ink4 transition-all rounded-md cursor-pointer text-white" onClick={() => handleInsertCharacter("4")}>4</button>
                  <button className="calc-btn text-base py-3 bg-ink3 border border-border-custom font-medium hover:bg-ink4 transition-all rounded-md cursor-pointer text-white" onClick={() => handleInsertCharacter("5")}>5</button>
                  <button className="calc-btn text-base py-3 bg-ink3 border border-border-custom font-medium hover:bg-ink4 transition-all rounded-md cursor-pointer text-white" onClick={() => handleInsertCharacter("6")}>6</button>
                  <button className="calc-btn py-3 bg-accent-custom/5 border border-accent-custom/15 text-accent-custom hover:border-accent-custom hover:bg-ink4 rounded-md transition-all font-bold text-base flex items-center justify-center" onClick={() => handleInsertCharacter("-")}>&minus;</button>
                  <button className="calc-btn text-sm py-3 bg-ink3 border border-border-custom text-green-custom hover:bg-ink4 hover:border-green-custom/30 transition-all rounded-md cursor-pointer font-semibold" onClick={() => handleInsertCharacter("Math.cbrt(")}>∛</button>

                  <button className="calc-btn text-base py-3 bg-ink3 border border-border-custom font-medium hover:bg-ink4 transition-all rounded-md cursor-pointer text-white" onClick={() => handleInsertCharacter("1")}>1</button>
                  <button className="calc-btn text-base py-3 bg-ink3 border border-border-custom font-medium hover:bg-ink4 transition-all rounded-md cursor-pointer text-white" onClick={() => handleInsertCharacter("2")}>2</button>
                  <button className="calc-btn text-base py-3 bg-ink3 border border-border-custom font-medium hover:bg-ink4 transition-all rounded-md cursor-pointer text-white" onClick={() => handleInsertCharacter("3")}>3</button>
                  <button className="calc-btn py-3 bg-accent-custom/5 border border-accent-custom/15 text-accent-custom hover:border-accent-custom hover:bg-ink4 rounded-md transition-all font-bold text-base flex items-center justify-center" onClick={() => handleInsertCharacter("+")}>+</button>
                  <button className="calc-btn text-[11px] py-3 bg-ink3 border border-border-custom text-green-custom hover:bg-ink4 hover:border-green-custom/30 transition-all rounded-md cursor-pointer font-semibold" onClick={() => handleInsertCharacter("1/(")}>1/x</button>

                  <button className="calc-btn text-sm py-3 bg-ink3 border border-border-custom hover:bg-ink4 rounded-md cursor-pointer text-green-custom transition-all" onClick={() => handleInsertCharacter("(")}>(</button>
                  <button className="calc-btn text-base py-3 bg-ink3 border border-border-custom font-semibold hover:bg-ink4 transition-all rounded-md cursor-pointer text-white" onClick={() => handleInsertCharacter("0")}>0</button>
                  <button className="calc-btn text-base py-3 bg-ink3 border border-border-custom hover:bg-ink4 rounded-md cursor-pointer text-green-custom transition-all" onClick={() => handleInsertCharacter(".")}>.</button>
                  <button className="calc-btn text-sm py-3 bg-ink3 border border-border-custom hover:bg-ink4 rounded-md cursor-pointer text-green-custom transition-all" onClick={() => handleInsertCharacter(")")}>)</button>
                  
                  {/* MASTER EVALUATION ELEMENT */}
                  <button className="calc-btn font-syne text-xl text-red-custom py-3 rounded-lg border border-red-custom/35 bg-red-custom/10 hover:bg-red-custom/20 hover:border-red-custom shadow-[0_0_12px_rgba(255,71,87,0.12)] hover:shadow-[0_0_20px_rgba(255,71,87,0.35)] cursor-pointer transition-all font-extrabold select-none outline-none" onClick={handleEvaluate}>=</button>
                </div>
              )}

              {/* 2. TRIGONOMETRICAL PANEL TRIG */}
              {activePanel === "trig" && (
                <div className="grid grid-cols-6 gap-1.5 p-0.5 animate-bounce-in font-mono" id="panel-kb-trig">
                  {[
                  ...
                  <button className="calc-btn text-sm text-white py-2.5 bg-ink3 border border-border-custom rounded-md hover:bg-ink4 transition-all" onClick={() => handleInsertCharacter("7")}>7</button>
                  <button className="calc-btn text-sm text-white py-2.5 bg-ink3 border border-border-custom rounded-md hover:bg-ink4 transition-all" onClick={() => handleInsertCharacter("8")}>8</button>
                  <button className="calc-btn text-sm text-white py-2.5 bg-ink3 border border-border-custom rounded-md hover:bg-ink4 transition-all" onClick={() => handleInsertCharacter("9")}>9</button>
                  <button className="calc-btn text-sm text-accent-custom py-2.5 bg-ink3 border border-border-custom rounded-md hover:bg-ink4" onClick={() => handleInsertCharacter("/")}>÷</button>
                  <button className="calc-btn text-sm text-accent-custom py-2.5 bg-ink3 border border-border-custom rounded-md hover:bg-ink4" onClick={() => handleInsertCharacter("*")}>&times;</button>
                  <button className="calc-btn text-sm text-accent-custom py-2.5 bg-ink3 border border-border-custom rounded-md hover:bg-ink4" onClick={() => handleInsertCharacter("**")}>xʸ</button>

                  <button className="calc-btn text-sm text-white py-2.5 bg-ink3 border border-border-custom rounded-md hover:bg-ink4 transition-all" onClick={() => handleInsertCharacter("4")}>4</button>
                  <button className="calc-btn text-sm text-white py-2.5 bg-ink3 border border-border-custom rounded-md hover:bg-ink4 transition-all" onClick={() => handleInsertCharacter("5")}>5</button>
                  <button className="calc-btn text-sm text-white py-2.5 bg-ink3 border border-border-custom rounded-md hover:bg-ink4 transition-all" onClick={() => handleInsertCharacter("6")}>6</button>
                  <button className="calc-btn text-xs text-accent-custom py-2.5 bg-ink3 border border-border-custom rounded-md hover:bg-ink4" onClick={() => handleInsertCharacter("-")}>&minus;</button>
                  <button className="calc-btn text-xs text-accent-custom py-2.5 bg-ink3 border border-border-custom rounded-md hover:bg-ink4" onClick={() => handleInsertCharacter("+")}>+</button>
                  <button className="calc-btn text-xs text-green-custom border border-border-custom/50 py-2.5 rounded-md" onClick={() => handleInsertCharacter("(")}>(</button>

                  <button className="calc-btn text-sm text-white py-2.5 bg-ink3 border border-border-custom rounded-md hover:bg-ink4 transition-all" onClick={() => handleInsertCharacter("1")}>1</button>
                  <button className="calc-btn text-sm text-white py-2.5 bg-ink3 border border-border-custom rounded-md hover:bg-ink4 transition-all" onClick={() => handleInsertCharacter("2")}>2</button>
                  <button className="calc-btn text-sm text-white py-2.5 bg-ink3 border border-border-custom rounded-md hover:bg-ink4 transition-all" onClick={() => handleInsertCharacter("3")}>3</button>
                  <button className="calc-btn text-sm text-white py-2.5 bg-ink3 border border-border-custom rounded-md hover:bg-ink4 transition-all" onClick={() => handleInsertCharacter("0")}>0</button>
                  <button className="calc-btn text-sm text-green-custom py-2.5 bg-ink3 border border-border-custom rounded-md hover:bg-ink4" onClick={() => handleInsertCharacter(".")}>.</button>
                  <button className="calc-btn bg-red-custom/10 text-red-custom text-sm font-bold border border-red-custom/35 rounded-lg active:scale-95" onClick={handleEvaluate}>=</button>

                  <button className="col-span-2 calc-btn text-red-custom text-xs bg-ink3 border border-border-custom p-2 rounded-md hover:border-red-custom" onClick={handleAllClear}>AC</button>
                  <button className="col-span-2 calc-btn text-red-custom text-xs bg-ink3 border border-border-custom p-2 rounded-md hover:border-red-custom" onClick={handleBackspace}>⌫</button>
                  <button className="col-span-2 calc-btn text-green-custom text-xs p-2" onClick={() => handleInsertCharacter(")")}>)</button>
                </div>
              )}

              {/* 3. LOG/EXPONENTIAL PANEL LOG */}
              {activePanel === "log" && (
                <div className="grid grid-cols-6 gap-1.5 p-0.5 animate-bounce-in" id="panel-kb-log">
                  {[
                  ...
                  <button className="calc-btn text-sm text-accent-custom py-2.5 bg-ink3 border border-border-custom rounded-md hover:bg-ink4" onClick={() => handleInsertCharacter("/")}>÷</button>
                  <button className="calc-btn text-sm text-accent-custom py-2.5 bg-ink3 border border-border-custom rounded-md hover:bg-ink4" onClick={() => handleInsertCharacter("*")}>&times;</button>
                  <button className="calc-btn text-sm text-red-custom py-2.5 bg-ink3 border border-border-custom rounded-md hover:border-red-custom" onClick={handleBackspace}>⌫</button>
                  ...
                  <button className="calc-btn text-xs text-accent-custom py-2.5 bg-ink3 border border-border-custom rounded-md hover:bg-ink4" onClick={() => handleInsertCharacter("-")}>&minus;</button>
                  <button className="calc-btn text-xs text-accent-custom py-2.5 bg-ink3 border border-border-custom rounded-md hover:bg-ink4" onClick={() => handleInsertCharacter("+")}>+</button>
                  <button className="calc-btn text-xs text-red-custom py-2.5 bg-ink3 border border-border-custom rounded-md hover:bg-red-custom/10" onClick={handleAllClear}>AC</button>
              ...
              {activePanel === "stats" && (
                <div className="space-y-4 animate-bounce-in" id="panel-kb-stats">
                  ...
                  >
                    {statsResult || "Enter values above, then run an operation"}
                  </div>
                </div>
              )}
              ...
              {activePanel === "formulas" && (
                <div className="space-y-4 animate-bounce-in" id="panel-kb-formulas">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-3 gap-1.5 font-mono select-none">
                      {[
                        { label: "½bh", formula: "(1/2)*b*h", desc: "Triangle Area" },
                        { label: "πr²", formula: "Math.PI*r**2", desc: "Circle Area" },
                        { label: "2πr", formula: "2*Math.PI*r", desc: "Circumference" },
                        { label: "C.I.", formula: "p*(1+r/100)**t", desc: "Compound Interest" },
                        { label: "S.I.", formula: "(p*r*t)/100", desc: "Simple Interest" },
                        { label: "Pyth.", formula: "Math.sqrt(a**2+b**2)", desc: "Hypotenuse" },
                        { label: "Sphere Vol", formula: "(4/3)*Math.PI*r**3", desc: "Volume Sphere" },
                        { label: "Cylinder Vol", formula: "Math.PI*r**2*h", desc: "Cylinder Volume" },
                        { label: "n(n+1)/2", formula: "n*(n+1)/2", desc: "Sum of natural numbers" },
                        { label: "l×b×h", formula: "l*b*h", desc: "Cuboid Volume" },
                        { label: "Cone Vol", formula: "(1/3)*Math.PI*r**2*h", desc: "Cone Volume" },
                        { label: "Cube SA", formula: "6*a**2", desc: "Cube Surface Area" }
                      ].map((item, bidx) => (
                        <button
                          key={bidx}
                          title={item.desc}
                          ...
                          onClick={() => handleInsertCharacter(`(${item.formula})`)}
                        >
                          {item.label}
                        </button>
                      ))}
          ...
          <div className="bg-ink2 border border-border-custom rounded-xl overflow-hidden select-none sticky top-5" id="history-sidebar">
